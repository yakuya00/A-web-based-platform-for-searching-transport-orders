...
export const createConnectionPool = ({ driver, events, idleTimeout, maximumConnectionAge, maximumPoolSize, minimumPoolSize, }) => {
    ...
    const clearIdleTimer = (connection) => {
    ...
    const isConnectionTooOld = (connection) => {
        const metadata = connectionMetadata.get(connection);
        if (!metadata) {
            return false;
        }
    ...
    const acquire = async () => {
        return tracer.startActiveSpan('slonik.connection.acquire', async (span) => {
            try {
                ...
                span.setAttribute('slonik.pool.connections.pending', pendingConnections.length);
                ...
                const addConnection = async () => {
                    ...
                    const connection = await pendingConnection.catch((error) => {
                        ...
                        if (index === -1) {
                            logger.error('Unable to find pendingConnection in `pendingConnections` array to remove.');
                        }
                        else {
                    ...
                    const onDestroy = () => {
                        ...
                        if (indexOfConnection === -1) {
                            throw new UnexpectedStateError('Unable to find connection in `connections` array to remove.');
                        }
                        else {
                    ...
                    const indexOfPendingConnection = pendingConnections.indexOf(pendingConnection);
