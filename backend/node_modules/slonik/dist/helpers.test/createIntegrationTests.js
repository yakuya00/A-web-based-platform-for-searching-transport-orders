...
import { BackendTerminatedError, CheckExclusionConstraintViolationError, CheckIntegrityConstraintViolationError, createNumericTypeParser, createPool, ForeignKeyIntegrityConstraintViolationError, IdleTransactionTimeoutError, InputSyntaxError, InvalidInputError, NotNullIntegrityConstraintViolationError, sql, StatementCancelledError, StatementTimeoutError, TupleMovedToAnotherPartitionError, UnexpectedForeignConnectionError, UnexpectedStateError, UniqueIntegrityConstraintViolationError, } from '../index.js';
import { setTimeout as delay } from 'node:timers/promises';
import * as sinon from 'sinon';
import { z } from 'zod';
export const createIntegrationTests = (test, driverFactory) => {
    ...
    test('produces error if multiple statements are passed as the query input (without parameters)', async (t) => {
        ...
        const error = await t.throwsAsync(pool.query(sql.unsafe `
        SELECT 1; SELECT 2;
      `));
    ...
    test('NotNullIntegrityConstraintViolationError identifies the table and column', async (t) => {
        ...
        const error = await t.throwsAsync(pool.any(sql.unsafe `
    ...
    test('properly handles terminated connections', async (t) => {
        ...
        await t.notThrowsAsync(pool.query(sql.unsafe `
    ...
    test('produces syntax error with the original SQL', async (t) => {
        ...
        const error = await t.throwsAsync(pool.any(sql.unsafe `SELECT WHERE`));
        t.true(error instanceof InputSyntaxError);
    ...
    `);
        ...
        t.is(await pool.oneFirst(sql.unsafe `
        SELECT name
        FROM person
    ...
    test('returns true if returns rows', async (t) => {
        ...
        t.true(await pool.exists(sql.unsafe `
        SELECT LIMIT 1
      `));
        await pool.end();
    });
    test('returns false if returns rows', async (t) => {
        ...
        t.false(await pool.exists(sql.unsafe `
        SELECT LIMIT 0
      `));
        await pool.end();
    });
    ...
    `);
        ...
        t.deepEqual(await pool.manyFirst(sql.unsafe `
    ...
    const testConcurrentTransactions = ({ expectedResult1, expectedResult2, isolationLevel, }) => {
