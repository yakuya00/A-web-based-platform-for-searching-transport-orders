...
const execNestedTransaction = async (parentLog, connection, clientConfiguration, handler, newTransactionDepth, transactionRetryLimit, eventEmitter, transactionId) => {
    await connection.query('SAVEPOINT slonik_savepoint_' + String(newTransactionDepth));
    if (!eventEmitter || !transactionId) {
        throw new UnexpectedStateError('Event emitter and transaction ID are required for nested transaction execution.');
    }
    ...
    try {
        const result = await handler(bindTransactionConnection(parentLog, connection, clientConfiguration, newTransactionDepth, eventEmitter, transactionId));
        return result;
    }
    catch (error) {
...
const retryNestedTransaction = async (parentLog, connection, clientConfiguration, handler, transactionDepth, transactionRetryLimit, eventEmitter, transactionId) => {
    ...
    while (remainingRetries-- > 0) {
        ...
        try {
            ...
            result = await execNestedTransaction(parentLog, connection, clientConfiguration, handler, transactionDepth, transactionRetryLimit, eventEmitter, transactionId);
...
export const nestedTransaction = async (parentLog, connection, clientConfiguration, handler, transactionDepth, transactionRetryLimit, eventEmitter, transactionId) => {
    const poolClientState = getPoolClientState(connection);
    ...
    try {
        ...
        return await execNestedTransaction(log, connection, clientConfiguration, handler, newTransactionDepth, transactionRetryLimit, eventEmitter, transactionId);
    }
    catch (error) {
        ...
        if (shouldRetry) {
            return await retryNestedTransaction(parentLog, connection, clientConfiguration, handler, newTransactionDepth, transactionRetryLimit, eventEmitter, transactionId);
        }
        else {
            throw error;
        }
