...
export const createPoolTests = (test, driverFactory) => {
    ...
    test('does not re-use connection if there was an unhandled error', async (t) => {
      ...
      SELECT pg_backend_pid();
    `);
        await t.throwsAsync(pool.query(sql.unsafe `
        SELECT 1 / 0;
      `));
        const secondConnectionPid = await pool.oneFirst(sql.unsafe `
      SELECT pg_backend_pid();
    ...
    test('connections are reset after they are released', async (t) => {
        ...
        SET slonik.foo = 'bar';
      `);
            t.is(await connection.oneFirst(sql.unsafe `
    ...
    `);
        ...
        await t.is(firstConnectionPid, await pool.oneFirst(sql.unsafe `
        SELECT pg_backend_pid();
      `));
        await t.throwsAsync(pool.query(sql.unsafe `
    ...
    const terminateBackend = async (dsn, pid) => {
    ...
    test('connections failing auth are not added to the connection pool', async (t) => {
        ...
        await superPool.query(sql.unsafe `
        CREATE ROLE ${sql.identifier([testUser])}
        WITH LOGIN SUPERUSER
        PASSWORD 'auth_change_test'
      `);
        // Connect as the new role
        const pool = await createPool(`postgres://${testUser}:auth_change_test@${connection.host}:${connection.port}/${connection.databaseName}`, {
            driverFactory,
            idleTimeout: 1_000,
            maximumPoolSize: 1,
        });
...
//# sourceMappingURL=createPoolTests.js.map