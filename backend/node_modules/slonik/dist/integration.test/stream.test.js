...
test('reading stream after a delay', async (t) => {
    ...
    await t.notThrowsAsync(pool.stream(sql.unsafe `
        SELECT *
...
test('untapped stream produces statement timeout', async (t) => {
    ...
    const onData = sinon.spy();
    const error = await t.throwsAsync(pool.stream(sql.unsafe `
      SELECT *
      FROM GENERATE_SERIES(1, 1000)
    `, (stream) => {
        setTimeout(() => {
...
test('streams rows', async (t) => {
  ...
  `);
    const messages = [];
    await pool.stream(sql.type(z.object({
        name: z.string(),
    })) `
      SELECT name
      FROM person
...
test('streams include notices', async (t) => {
    ...
    $$;
    `);
    const result = await pool.stream(sql.unsafe `
      SELECT *
      FROM test_notice(${10})
    `, (stream) => {
        stream.on('data', () => { });
...
test('frees connection after destroying a stream', async (t) => {
    const pool = await createPool(t.context.dsn);
    await t.throwsAsync(pool.stream(sql.unsafe `
