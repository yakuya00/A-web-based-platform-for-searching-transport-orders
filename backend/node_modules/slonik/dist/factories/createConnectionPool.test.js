...
const acquireConnections = async (pool, count) => {
    const connections = [];
...
const releaseConnections = async (connections) => {
...
const waitForPoolStabilization = async (pool, maxWait = 200, clock) => {
    if (clock) {
...
class MockDriverClient extends EventEmitter {
    queryBehavior;
    _id;
    _state = 'IDLE';
    constructor() {
...
const assertPoolState = (t, pool, expected) => {
    const state = pool.state();
    for (const [key, value] of Object.entries(expected)) {
        t.is(state[key], value, `state.${key} should be ${value}`);
    }
};
const createTestPool = (driver, options = {}) => {
...
test('concurrent pool operations handle correctly', async (t) => {
    ...
    // Acquire operations
    for (let index = 0; index < 10; index++) {
        operations.push(pool.acquire().then(async (conn) => {
