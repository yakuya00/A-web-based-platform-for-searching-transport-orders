...
test('throws a descriptive error if query is empty', async (t) => {
    const error = await t.throwsAsync(async () => {
        return await executeQuery(t.context.logger, t.context.connection, createClientConfiguration(), {
            sql: '',
            values: [],
...
test('retries an implicit query that failed due to a transaction error', async (t) => {
    ...
    const result = await executeQuery(t.context.logger, t.context.connection, createClientConfiguration(), {
        sql: 'SELECT 1 AS foo',
        values: [],
...
test('returns the thrown transaction error if the retry limit is reached', async (t) => {
    ...
    const error = await t.throwsAsync(executeQuery(t.context.logger, t.context.connection, {
        ...clientConfiguration,
        queryRetryLimit: 1,
    }, {
        sql: 'SELECT 1 AS foo',
...
test('transaction errors are not handled if the function was called by a transaction', async (t) => {
    ...
    const clientConfiguration = createClientConfiguration();
    const error = await t.throwsAsync(executeQuery(t.context.logger, connection, {
        ...clientConfiguration,
        queryRetryLimit: 1,
    }, {
        sql: 'SELECT 1 AS foo',
