...
const raceError = async (connection, routine) => {
    const connectionErrorPromise = defer();
    const onError = (error) => {
...
export const createConnection = async (parentLog, pool, clientConfiguration, connectionType, connectionHandler, poolHandler, query = null) => {
    const { state } = pool.state();
    const poolId = pool.id();
    if (state === 'ENDING') {
        throw new UnexpectedStateError('Connection pool is being shut down. Cannot create a new connection.');
    }
    if (state === 'ENDED') {
        throw new UnexpectedStateError('Connection pool has been shut down. Cannot create a new connection.');
    }
    return await tracer.startActiveSpan('slonik.createConnection', async (span) => {
        ...
        try {
            ...
            const { connectionId, poolId: poolIdFromState } = getPoolClientState(connection);
